...
.nf-app .tab:hover{color:var(--nf-text)!important;}
...
.nf-app .nav-btn:hover{background:var(--nf-border)!important;border-color:var(--nf-border-strong)!important;}
.nf-app .nav-link:hover{color:var(--nf-text)!important;}
...
function SidebarRow({ ls }: { ls: SidebarLesson }) {
  ...
  if (ls.status === "done") {
    ...
    statusIcon = <Play className="size-2.5" style={{ marginLeft: 1 }} aria-hidden="true" />; titleColor = "var(--nf-text)"; titleWeight = 700;
  } else {
  ...
  return (
    ...
    >
      ...
      <span style={{ fontSize: 12, color: "var(--nf-muted)", fontVariantNumeric: "tabular-nums" }}>{ls.dur}</span>
    </div>
  );
}
...
export function NoryFlowLesson({
  ...
  coursePercent, modules, completed, onToggleComplete, onPrev, onNext, onBack, onSignOut, tenantSlug, userId,
}: Props) {
  ...
  const tabStyle = (active: boolean): React.CSSProperties => ({ fontSize: 14, fontWeight: 600, color: active ? "var(--nf-text)" : "var(--nf-muted)" });
  ...
  return (
    <div className="nf-app" data-theme={isDark ? "dark" : "light"} style={{ minHeight: "100vh", background: "var(--nf-bg)", color: "var(--nf-text)", fontFamily: "'Manrope',system-ui,sans-serif", ["--nf-accent" as string]: accentVal, ...noryFlowVars(isDark) } as React.CSSProperties}>
      <style>{CSS}</style>

      {/* TOP BAR */}
      <header className="nf-top" style={{ position: "sticky", top: 0, zIndex: 60, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, background: "var(--nf-topbar)", backdropFilter: "blur(16px)", borderBottom: "1px solid var(--nf-border)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 18, minWidth: 0 }}>
          <button type="button" className="nav-btn" onClick={onBack} aria-label="Voltar ao curso" style={{ display: "flex", alignItems: "center", gap: 9, color: "var(--nf-text)", background: "var(--nf-hover)", border: "1px solid var(--nf-border)", borderRadius: 100, padding: "9px 16px", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "'Manrope'", flex: "0 0 auto" }}><ArrowLeft className="size-4" aria-hidden="true" /><span className="nf-back-label">Voltar ao curso</span></button>
          <div style={{ display: "flex", alignItems: "center", gap: 9, minWidth: 0 }}>
            <WorkspaceAvatar iconUrl={iconUrl} iconName={iconName} iconColor={iconColor} size="sm" />
            <span style={{ fontFamily: "'Sora'", fontWeight: 600, fontSize: 15, color: "var(--nf-muted)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{courseTitle}</span>
          </div>
        </div>
        <NoryFlowControls
      ...
      <div className="player-layout nf-body" style={{ maxWidth: 1320, margin: "0 auto" }}>
        ...
        <main>
          ...
          <div style={{ position: "relative", width: "100%", aspectRatio: "16/9", borderRadius: 16, overflow: "hidden", background: "radial-gradient(120% 130% at 50% 40%,#13203B,#06080D)", border: "1px solid var(--nf-border)" }}>
            {player}
          </div>
          ...
          <div style={{ marginTop: 22 }}>
            ...
            <div style={{ fontSize: 14, color: "var(--nf-muted)" }}>{lessonMeta}</div>
          </div>

          {/* abas */}
          <div role="tablist" style={{ display: "flex", alignItems: "center", gap: 28, marginTop: 24, borderBottom: "1px solid var(--nf-border)" }}>
            {hasDesc && (
          ...
          <div style={{ marginTop: 22, minHeight: 140 }}>
            {tab === "desc" && hasDesc && (
              <div className={`prose max-w-[640px] ${isDark ? "prose-invert" : ""}`} style={{ color: "var(--nf-text2)" }} dangerouslySetInnerHTML={{ __html: descriptionHtml! }} />
            )}
            {tab === "files" && <div className={isDark ? "dark" : ""}>{filesNode}</div>}
            {tab === "links" && <div className={isDark ? "dark" : ""}>{linksNode}</div>}
          </div>

          {/* footer actions */}
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, marginTop: 32, paddingTop: 24, borderTop: "1px solid var(--nf-border)", flexWrap: "wrap" }}>
            ...
            <div className="nf-footer-nav" style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <button type="button" className="nav-btn" onClick={onPrev} disabled={!onPrev} style={{ display: "flex", alignItems: "center", gap: 8, color: "var(--nf-text)", background: "var(--nf-hover)", border: "1px solid var(--nf-border)", borderRadius: 100, padding: "12px 20px", fontSize: 14, fontWeight: 600, cursor: onPrev ? "pointer" : "not-allowed", opacity: onPrev ? 1 : 0.4, fontFamily: "'Manrope'" }}><ArrowLeft className="size-4" aria-hidden="true" /> Aula anterior</button>
              <button type="button" className="nav-btn" onClick={onNext} disabled={!onNext} style={{ display: "flex", alignItems: "center", gap: 8, color: "var(--nf-text)", background: "var(--nf-hover)", border: "1px solid var(--nf-border)", borderRadius: 100, padding: "12px 20px", fontSize: 14, fontWeight: 600, cursor: onNext ? "pointer" : "not-allowed", opacity: onNext ? 1 : 0.4, fontFamily: "'Manrope'" }}>Próxima aula <ArrowRight className="size-4" aria-hidden="true" /></button>
            </div>
          </div>
        </main>

        {/* SIDEBAR */}
        <aside className="course-sidebar" style={{ position: "sticky", top: 90, maxHeight: "calc(100vh - 110px)", background: "linear-gradient(160deg,var(--nf-surface),var(--nf-surface2))", border: "1px solid var(--nf-border)", borderRadius: 16, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          <div style={{ padding: "18px 20px 14px", borderBottom: "1px solid var(--nf-border)" }}>
            <div style={{ fontFamily: "'Space Grotesk'", textTransform: "uppercase", letterSpacing: ".22em", fontSize: 11, fontWeight: 600, color: "var(--nf-muted)", marginBottom: 10 }}>Aulas do curso</div>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <span style={{ fontSize: 12, color: "var(--nf-muted)", fontWeight: 600, whiteSpace: "nowrap" }}>{coursePercent}%</span>
            </div>
          </div>
            ...
            {modules.map((mod) => (
              <div key={mod.id}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px 8px" }}>
                  ...
                  <span style={{ fontSize: 11, color: "var(--nf-muted)" }}>{mod.done}/{mod.count}</span>
                </div>
                {mod.lessons.map((ls) => (
