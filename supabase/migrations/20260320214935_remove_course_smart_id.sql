-- Remove smart_id from courses: column, triggers, functions.
-- Courses now use UUID (admin) and slug (public URLs) exclusively.
-- smart_id remains only on checkouts.

-- 1. Drop triggers (must come before dropping functions)
DROP TRIGGER IF EXISTS trg_set_course_smart_id ON public.courses;
DROP TRIGGER IF EXISTS trg_prevent_course_smart_id_update ON public.courses;

-- 2. Drop functions
DROP FUNCTION IF EXISTS public.set_course_smart_id();
DROP FUNCTION IF EXISTS public.prevent_course_smart_id_update();
DROP FUNCTION IF EXISTS public.generate_course_smart_id();

-- 3. Drop column (constraint drops automatically with the column)
ALTER TABLE public.courses DROP COLUMN IF EXISTS smart_id;

-- 4. Update duplicate_course RPC: remove smart_id from RETURNING and result JSON
...
BEGIN
  ...
  -- 3. Insert new course (handle_new_course trigger creates a default module)
  ...
  -- 6. Return result with media map (no more smart_id)
