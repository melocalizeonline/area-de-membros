-- Separate storage for the "raw HTML" lesson content mode.
--
-- Previously, the toggle between the visual editor (TipTap) and the raw HTML
-- editor shared a single `content` column. Switching modes overwrote the
-- other one's text, which isn't what authors want: they expect each mode to
-- keep its own draft so they can switch back and forth without loss.
--
-- This migration:
--   1. Adds lessons.custom_html (nullable text) for the raw-HTML mode
--   2. Updates save_lesson_editor to accept p_custom_html alongside
--      p_content_html — neither overwrites the other
--   3. Updates duplicate_course to carry custom_html to the copy

-- ═══════════════════════════════════════════════════════════════════════
-- 1. Add custom_html column
-- ═══════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════
-- 2. Recreate save_lesson_editor with p_custom_html
--    (based on 20260414210000_lesson_content_mode.sql — only the lessons
--    UPDATE section and the signature change)
-- ═══════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- Normalize content_mode (defensive — the CHECK constraint would
  ...
  --    content (rich editor output) and custom_html (raw HTML) are stored
  --    independently. content_mode decides which one is rendered to students.
...
-- ═══════════════════════════════════════════════════════════════════════
-- 3. Recreate duplicate_course carrying custom_html over to the copy
--    (based on 20260414210000_lesson_content_mode.sql — adds custom_html)
-- ═══════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  LOOP
    ...
    FOR v_les IN
      SELECT id, title, description, content, content_mode, custom_html, thumbnail_url, video_url,
             video_provider, duration_seconds, sort_order, is_active
        FROM lessons
       WHERE module_id = v_mod.id
       ORDER BY sort_order
    LOOP
      INSERT INTO lessons (module_id, title, description, content, content_mode, custom_html, thumbnail_url,
                           video_url, video_provider, duration_seconds, sort_order, is_active)
      VALUES (v_new_module_id, v_les.title, v_les.description, v_les.content, v_les.content_mode, v_les.custom_html,
