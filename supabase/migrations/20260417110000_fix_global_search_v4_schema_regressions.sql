-- ============================================================
-- Fix global_search v4 regressions after schema changes
-- - products.slug was removed
-- - products.gateway_product_ids was removed
-- - admin routes resolve resources by public_id, not UUID
-- ============================================================
...
BEGIN
  ...
  -- Assets (search by title, description, mime_type, public_id)
  (SELECT
  ...
  -- Customers (search by name, email, public_id)
  (SELECT
  ...
  -- Orders (search by public_id, order_number, gateway_external_id, customer name/email, product name)
  (SELECT
...
COMMENT ON FUNCTION public.global_search(uuid, text) IS 'v4 hotfix: remove refs to dropped product columns and restore admin URLs/search by public_id';
