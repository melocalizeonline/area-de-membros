-- ============================================================
-- RPC orquestradora para import de customers via CSV
-- Recebe rows já validadas/normalizadas pelo edge function.
-- Cria batch, upsert customers, cria orders — tudo transacional.
-- ============================================================
...
CREATE OR REPLACE FUNCTION public.run_customer_csv_import(
  ...
  p_filename     text,
  p_rows         jsonb
  -- cada item: { line, email, name, phone, phone_country_code, first_name, last_name,
  --              city, state, country, document, document_type, external_customer_id,
  --              resolved_products: [{product_id, product_public_id}] }
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  -- ─── 1. Criar batch ─────────────────────────────────────────
  INSERT INTO customer_import_batches (tenant_id, imported_by, filename, status, total_rows)
  VALUES (p_tenant_id, p_imported_by, p_filename, 'processing', v_total_rows)
  ...
  BEGIN
    ...
    LOOP
      ...
      ELSE
        ...
        -- We build a dynamic update via simple conditionals
        -- Name
      ...
      -- ── Create orders for resolved products ──────────────────
      IF v_row -> 'resolved_products' IS NOT NULL AND jsonb_array_length(v_row -> 'resolved_products') > 0 THEN
      ...
      -- If customer existed but new orders were created → updated
      IF v_action = 'existing' AND jsonb_array_length(v_orders_created_arr) > 0 THEN
        v_action := 'updated';
        v_skipped_count := v_skipped_count - 1;
        v_updated_count := v_updated_count + 1;
      END IF;
...
REVOKE EXECUTE ON FUNCTION public.run_customer_csv_import(uuid, uuid, text, jsonb) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.run_customer_csv_import(uuid, uuid, text, jsonb) FROM anon;
REVOKE EXECUTE ON FUNCTION public.run_customer_csv_import(uuid, uuid, text, jsonb) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.run_customer_csv_import(uuid, uuid, text, jsonb) TO service_role;
