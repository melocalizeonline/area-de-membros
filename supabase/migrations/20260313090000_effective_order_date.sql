-- ============================================================
-- Orders: effective order date from gateway webhook
-- ============================================================

ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS gateway_order_created_at TIMESTAMPTZ;

COMMENT ON COLUMN public.orders.gateway_order_created_at IS
  'Original order creation timestamp reported by the payment gateway webhook. Falls back to orders.created_at when NULL.';

CREATE INDEX IF NOT EXISTS idx_orders_tenant_effective_order_at
  ON public.orders (
    tenant_id,
    (COALESCE(gateway_order_created_at, created_at)) DESC,
    id DESC
  );

CREATE INDEX IF NOT EXISTS idx_orders_tenant_status_effective_order_at
  ON public.orders (
    tenant_id,
    status,
    (COALESCE(gateway_order_created_at, created_at)) DESC
...
CREATE FUNCTION public.get_tenant_orders(
  p_tenant_id UUID,
