-- Lesson Links: reuse lesson_blocks (type='link') for external URLs per lesson.
-- Updates save_lesson_editor RPC to accept p_links JSONB parameter.
-- Updates duplicate_course RPC to clone lesson_blocks (+ fixes provider_asset_id column name).
-- No new tables. No backfill needed. Fully backwards-compatible.

-- ═══════════════════════════════════════════════════════════════════════
-- 1. RECREATE save_lesson_editor with p_links support
-- ═══════════════════════════════════════════════════════════════════════
...
CREATE OR REPLACE FUNCTION public.save_lesson_editor(
  ...
  -- Gumlet: via internal asset library
  p_video_asset_id UUID DEFAULT NULL,
  -- Generic provider: Vimeo, YouTube
  ...
  -- External links (NEW) — array of {label, url}
...
-- ═══════════════════════════════════════════════════════════════════════
-- 2. UPDATE duplicate_course: add lesson_blocks cloning
--    Based on current version from 20260331120000_rpcs_return_public_id.sql
--    Also fixes provider_asset_id column name (was gumlet_asset_id in prior version)
-- ═══════════════════════════════════════════════════════════════════════
