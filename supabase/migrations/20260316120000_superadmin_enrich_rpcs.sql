-- ============================================================
-- Superadmin: Enrich existing RPCs + new tenant_users RPC
-- ============================================================

-- 1. Tenants: add owner_name, owner_email, courses_count
...
BEGIN
  ...
  WITH tenant_stats AS (
    SELECT
      ...
      ow.oname AS owner_name,
      ow.oemail AS owner_email
    FROM tenants t
    LEFT JOIN LATERAL (
    ...
    LEFT JOIN LATERAL (
      SELECT
        COALESCE(pr.name, au.email) AS oname,
        au.email AS oemail
...
-- 2. Customers: add user_id, last_sign_in_at, email_confirmed_at, orders_count
DROP FUNCTION IF EXISTS public.get_superadmin_customers(TEXT, UUID, INTEGER, INTEGER);
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(oc.cnt, 0) AS orders_count,
    c.created_at,
    COUNT(*) OVER() AS total_count
  FROM customers c
  ...
  LEFT JOIN LATERAL (
    SELECT COUNT(*) AS cnt FROM orders o WHERE o.customer_id = c.id
  ) oc ON TRUE
...
-- 3. New: Tenant Users cross-tenant
...
BEGIN
  ...
  SELECT
    ...
    tu.tenant_id,
    t.name AS tenant_name,
    COALESCE(pr.name, au.email) AS name,
    au.email,
    tu.role::TEXT,
  ...
  WHERE (p_tenant_id IS NULL OR tu.tenant_id = p_tenant_id)
    AND (
      v_search IS NULL
      OR COALESCE(pr.name, '') ILIKE '%' || v_search || '%'
