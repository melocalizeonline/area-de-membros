-- ============================================================
-- Migration: fix CSV import after idempotency index rescope
--
-- Migration 20260419170000 dropped the global unique index on
-- orders.idempotency_key and replaced it with (tenant_id, idempotency_key).
-- The run_customer_csv_import RPC (last defined in 20260404100001) still
-- uses ON CONFLICT (idempotency_key), which no longer has a matching
-- unique index and will fail on the next CSV import run with:
--   ERROR: there is no unique or exclusion constraint matching the
--   ON CONFLICT specification
--
-- Recreate the RPC identically, changing only the ON CONFLICT target.
-- ============================================================

DROP FUNCTION IF EXISTS public.run_customer_csv_import(uuid, uuid, text, jsonb, text);
  ...
  BEGIN
    ...
    LOOP
      ...
      ELSE
        ...
        IF v_external_id IS NOT NULL AND v_existing.external_id IS NULL THEN
          v_has_updates := true;
        ELSIF v_external_id IS NOT NULL AND v_existing.external_id IS NOT NULL AND v_external_id <> v_existing.external_id THEN
          v_warnings := v_warnings || jsonb_build_array('Divergência: external_id: CSV="' || v_external_id || '" vs DB="' || v_existing.external_id || '"');
        END IF;
        ...
        IF v_has_updates THEN
          UPDATE customers SET
            ...
            external_id          = COALESCE(customers.external_id, v_external_id)
