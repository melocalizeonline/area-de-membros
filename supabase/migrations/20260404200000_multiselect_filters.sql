-- Allow comma-separated values for p_status and p_product_id filters
-- e.g. p_status = 'pending,approved' or p_product_id = 'uuid1,uuid2'

-- ─── 1. get_tenant_orders ───

DROP FUNCTION IF EXISTS public.get_tenant_orders(UUID, TEXT, INTEGER, INTEGER, TEXT, TEXT, UUID, TIMESTAMPTZ, TIMESTAMPTZ);
...
CREATE OR REPLACE FUNCTION public.get_tenant_orders(
  ...
  p_status TEXT DEFAULT NULL,        -- comma-separated statuses e.g. 'pending,approved'
  p_product_id TEXT DEFAULT NULL,    -- comma-separated UUIDs  e.g. 'uuid1,uuid2'
...
DROP FUNCTION IF EXISTS public.get_order_metrics(UUID, TEXT, TEXT, UUID, TIMESTAMPTZ, TIMESTAMPTZ);
