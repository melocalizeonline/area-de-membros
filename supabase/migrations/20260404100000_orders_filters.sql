-- Add server-side filters (status, product, date range) to orders RPCs
-- No new tables or columns — only function signature changes.

-- ─── 1. get_tenant_orders — add p_status, p_product_id, p_start_at, p_end_at ───
...
-- ─── 2. get_order_metrics — accept same filters ───

DROP FUNCTION IF EXISTS public.get_order_metrics(UUID);
...
BEGIN
  ...
  IF v_has_filter THEN
    -- Filtered mode: return total_orders + revenue_total for the filtered set
  ...
  ELSE
    -- Default mode: same as original (approved/completed, exclude csv_import)
    SELECT JSON_BUILD_OBJECT(
      'total_orders', COUNT(*),
