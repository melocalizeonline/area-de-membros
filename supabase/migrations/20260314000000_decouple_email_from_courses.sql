-- ============================================================
-- Migration: Desacoplar email pós-compra da lógica de cursos
--
-- Problema: email dependia de courses_granted > 0, bloqueando
-- envio para produtos não-curso e quando triggers antecipam acesso.
-- Solução: RPC resolve identidade para QUALQUER tipo de produto;
-- email depende de order ativa + identidade resolvida.
-- ============================================================

-- 1. Adicionar 'skipped' ao enum de status de email
ALTER TYPE public.email_log_status ADD VALUE IF NOT EXISTS 'skipped';

-- 2. Refatorar RPC: resolver identidade ANTES de checar tipo de benefício
...
BEGIN
  ...
  IF v_user_id IS NULL THEN
    ...
    ELSE
      ...
      -- Funciona para QUALQUER tipo de produto (não apenas cursos)
      RETURN jsonb_build_object(
  ...
  -- 5. Se produto não é de cursos: retornar com identidade resolvida, sem materializar acesso
  IF v_product.benefit != 'courses' THEN
    -- Garantir role customer mesmo para não-cursos
  ...
  IF NOT v_product_has_courses THEN
    -- Antes era erro fatal — agora retorna ok com note (não bloqueia email)
    INSERT INTO user_roles (user_id, role)
    VALUES (v_user_id, 'customer')
    ON CONFLICT DO NOTHING;
